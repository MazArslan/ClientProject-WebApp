...
READS SQL DATA
  BEGIN
    ...
    IF EXISTS(SELECT *
              FROM configuration
              WHERE name = 'image_ranking_alg' AND value = 0)
    THEN
      RETURN get_image_ranking_advanced(image_id_given);
    ELSE
      RETURN get_image_ranking_simple(image_id_given);
    END IF;
  END //
DELIMITER ;
DELIMITER //

CREATE FUNCTION get_image_ranking_advanced(image_id_given INT)
  RETURNS DOUBLE
READS SQL DATA
  BEGIN
    -- tracks if the cursor is done
...
DELIMITER //
CREATE FUNCTION get_image_ranking_simple(image_id_given INT)
  RETURNS DOUBLE
READS SQL DATA
  BEGIN
    DECLARE result DOUBLE default 0;
    SELECT IFNULL(AVG(is_vote_up), 0)
    FROM votes
    WHERE FK_image_id = image_id_given
    INTO result;
    RETURN result;
